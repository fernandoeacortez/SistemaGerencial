...
const tabela = document.querySelector("#tabelaAlunos tbody"); // caso queira reusar a tabela
...
// Renderizar tabela (caso mantenha tabela oculta ou use futuramente)
...
// Atualiza título da guia selecionada e mostra seções apropriadas
function atualizarTituloGuia(nomeGuia) {
  tituloGuiaSelecionada.textContent = nomeGuia;
  form.classList.add("oculto");
  document.getElementById("secaoRelatorios").classList.add("oculto");
  ...
  switch (nomeGuia) {
    case "Cadastro de Aluno":
      ...
      break;
    case "Relatórios":
      document.getElementById("secaoRelatorios").classList.remove("oculto");
      gerarRelatorio();
      break;
    default:
      break;
...
function gerarRelatorio() {
  ...
  const relTotal = document.getElementById("relTotalAlunos");
  const relPlanoLista = document.getElementById("relatorioPorPlano");

  relTotal.textContent = totalAlunos;
  relPlanoLista.innerHTML = "";

  const contagem = {};
  alunos.forEach(aluno => {
    contagem[aluno.plano] = (contagem[aluno.plano] || 0) + 1;
  });

  for (const plano in contagem) {
    const li = document.createElement("li");
    li.textContent = `${plano}: ${contagem[plano]} aluno(s)`;
    relPlanoLista.appendChild(li);
  }
}
...
menuLinks.forEach(link => {
  link.addEventListener("click", (e) => {
    ...
    // Ativa visualmente o link clicado (opcional)
    menuLinks.forEach(l => l.classList.remove("ativa"));
